class Admin::PatronsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @patron.save
        format.html { redirect_to admin_patron_path(@patron), notice: 'Patron was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @patron.update_attributes(params[:patron])
        format.html { redirect_to admin_patron_path(@patron), notice: 'Patron was successfully updated.' }
        format.json { head :ok }
      else
